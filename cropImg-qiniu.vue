		...
		<div class="col-md-10 col-sm-10 col-xs-10 imgJCrop">
			...
			<div class="myVar" v-show="isShowBox">
				...
				<div class="tab-content" :class="{isNeedSys:option.isNeedSystemImg}">
					<div v-show="isShowSys&&option.isNeedSystemImg" class="SystemPhoto">
						<ol>
					...
					<div v-show="!isShowSys" class="localUp">
						<div class="zykvar1" v-show="isShowInputFile">
							<span id="container" class="btnSpan">
								<button type="button" class="btn btn-default" id="pickfiles">
									<i class="iconfont">&#xe655;</i>选择本地照片
								</button>
							</span>
						</div>
						<div v-show="!isShowInputFile" class="jcrop-holder">
		...
		methods:{
				...
				if(file){		
					if (_this.option.maxSize) {//限制了大小
						...
						if (fileSize >= (_this.option.maxSize.substr(0, _this.option.maxSize.length - 2))) {
							return {bool:false,msg:"图片大小必须小于" + _this.option.maxSize + "，请重新上传!"};
						};
					};
					if(_this.option.limitType.length>0){
						...
						if (_this.option.limitType.indexOf(ext) == -1) {
							return {bool:false,msg:'格式不正确！'};
						};
					};
					_this.isShowInputFile=false;//必须在前端判断是否符合条件之后再执行
					var fileItem = file.getNative(),url = window.URL || window.webkitURL || window.mozURL;
					var img = new Image();
			...
			qiniuCrop:function(){
				...
				_this.uploader = Qiniu.uploader({
					...
					max_file_size: _this.option.maxSize,// 最大文件体积限制
					flash_swf_url: '../../plupload/js/Moxie.swf',
					dragdrop: false,// 开启可拖曳上传
					chunk_size: '4mb',// 分块上传时，每块的体积
					multi_selection: !(mOxie.Env.OS.toLowerCase() === "ios"),
					uptoken_url:'/api/qn/getUploadToken',
					domain: 'http://pic.cgt.hulabanban.com/',//后台给  这里只是测试用
					get_new_uptoken: false,// 设置上传文件的时候是否每次都重新获取新的uptoken
					auto_start: false,// 选择文件后自动上传，若关闭需要自己绑定事件触发上传
					log_level: 5,
					init: {
						'FilesAdded': function(up, files) {
							var _res=_this.inspect(files);//检查是否符合上传要求
							if(!_res.bool){								
						...
						'FileUploaded': function(up, file, info) {
							...
							var _msg='!'+_this.cropSize.w+'x'+_this.cropSize.h+'a'+_this.cropSize.x+'a'+_this.cropSize.y;
							var _zoom=_this.cropSize.currentW+'x'+_this.cropSize.currentH;
							var imgLink = Qiniu.imageMogr2({
								thumbnail: _zoom,   // 缩放操作参数
								crop: _msg,  // 裁剪操作参数
							}, json.key);
							if(imgLink){
								_this.addImg(imgLink);
							}else{
								this.$store.commit('showMsgTip',{
									content:'上传失败！',
									model:1
